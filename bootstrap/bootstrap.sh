...
imageServer=$(az acr show --resource-group ffsync --name ffsync --query loginServer --output tsv)
...
az container create --resource-group $AZURE_RESOURCE_GROUP --name $AZURE_RESOURCE_GROUP\
 ...
 --environment-variables SYNCSERVER_SQLURI="sqlite:////home/ffsync/syncserver.db" FF_SYNCSERVER_PUBLIC_URL="$($AZURE_RESOURCE_GROUP).$($AZURE_LOCATION).azurecontainer.io" 
